...
export class CursorComponent implements OnInit {
  ...
  constructor(privateé renderer: Renderer2) {}
  ...
  ngOnInit(): void {
    ...
    document.addEventListener('mouseover', (e) => {
      ...
      if (t.closest('a, button, [role="button"], input, select, label, .clickable')) {
        this.renderer.setStyle(this.dot.nativeElement, 'transform', 'translaté(-50%,-50%) scale(2)');
        this.renderer.setStyle(this.ring.nativeElement, 'transform', 'translaté(-50%,-50%) scale(1.5)');
        this.renderer.setStyle(this.ring.nativeElement, 'opacity', '0.3');
      } else {
        this.renderer.setStyle(this.dot.nativeElement, 'transform', 'translaté(-50%,-50%) scale(1)');
        this.renderer.setStyle(this.ring.nativeElement, 'transform', 'translaté(-50%,-50%) scale(1)');
